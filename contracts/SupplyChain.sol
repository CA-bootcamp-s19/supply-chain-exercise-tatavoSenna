...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  function addItem(string memory _name, uint _price) public returns(bool){
    items[skuCount] = Item({name: _name, sku: skuCount, price: _price, state: States.ForSale, seller: msg.sender, buyer: address(0)});
  ...
  function shipItem(uint sku)
    ...
    isSeller(sku)
  {
    items[sku].state = States.Shipped;
    emit LogShipped(sku);
  }
  ...
  function receiveItem(uint sku)
    ...
    isBuyer(sku)
  {
    items[sku].state = States.Received;
